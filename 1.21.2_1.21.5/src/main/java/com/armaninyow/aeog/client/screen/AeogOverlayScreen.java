...
public class AeogOverlayScreen {
	...
	private static final Identifier BTN_MOD   = id(TEX + "mod_button_disabled.png");
	private static final Identifier BTN_MOD_H = id(TEX + "mod_button_enabled.png");
	...
	public record ModdedCategory(
		String id,
		List<net.minecraft.item.Item> items,
	...
	private static List<ModdedCategory> buildModdedCategories() {
		...
		for (net.minecraft.item.Item item : List.of(
			...
			vanillaIds.add(net.minecraft.registry.Registries.ITEM.getId(item).toString());
		}
		...
		var enchantReg = mc.world.getRegistryManager().getOrThrow(RegistryKeys.ENCHANTMENT);
		var itemReg    = net.minecraft.registry.Registries.ITEM;

		Map<String, List<net.minecraft.item.Item>> byEnchantSet = new LinkedHashMap<>();
		Map<String, List<String>> enchantSetKeys = new LinkedHashMap<>();

		for (net.minecraft.item.Item item : itemReg) {
			net.minecraft.util.Identifier itemId = itemReg.getId(item);
			...
			for (var enchEntry : enchantReg.streamEntries().toList()) {
				var ench = enchEntry.value();
				net.minecraft.item.ItemStack testStack = new net.minecraft.item.ItemStack(item);
				if (ench.isSupportedItem(testStack)) {
					net.minecraft.util.Identifier enchId = enchEntry.registryKey().getValue();
		...
		for (Map.Entry<String, List<net.minecraft.item.Item>> entry : byEnchantSet.entrySet()) {
			List<net.minecraft.item.Item> items = entry.getValue();
			List<String> enchants = enchantSetKeys.get(entry.getKey());
			net.minecraft.util.Identifier firstId = itemReg.getId(items.get(0));
	...
	private ItemStack vanillaStackForCategory(String category) {
		net.minecraft.item.Item item = switch (category) {
	...
	private static String toInternalEnchantKey(net.minecraft.util.Identifier enchId) {
	...
	private int getModdedEnchantMaxLevel(String enchantKey) {
		...
		net.minecraft.util.Identifier enchId = mcId.contains(":")
			? net.minecraft.util.Identifier.of(mcId)
			: Identifier.of("minecraft", mcId);
		var entry = reg.getEntry(enchId);
	...
	private net.minecraft.item.ItemStack hoveredMatchedStack = null;
	...
	private void renderPhaseOne(DrawContext ctx, int px, int py,
		...
		ctx.drawTexture(RenderLayer::getGuiTextured, P1_CONTAINER, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		...
		if (com.armaninyow.dibs.config.AeogConfig.showModButtonPhase1) {
			...
			ctx.drawTexture(RenderLayer::getGuiTextured, modMode ? BTN_MOD_H : BTN_MOD,
			...
			if (inBounds(mx, my, modBtnAbsX, modBtnAbsY, MOD_BTN_SIZE, MOD_BTN_SIZE)) {
				ctx.drawTooltip(tr, Text.literal(modMode ? "Switch to Vanilla Items" : "Switch to Modded Items"), mx, my);
			}
		}
		if (modMode) {
		...
		ctx.drawTexture(RenderLayer::getGuiTextured, optHov ? BTN_OPT_H : BTN_OPT,
	...
	private void renderPhaseOneVanilla(DrawContext ctx, int px, int py,
	                                    int mx, int my, TextRenderer tr) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			ctx.drawTexture(RenderLayer::getGuiTextured, (hov || sel) ? BTN_ITEM_H : BTN_ITEM,
	...
	private void renderPhaseOneModded(DrawContext ctx, int px, int py,
	                                   int mx, int my, TextRenderer tr) {
		...
		if (cats.isEmpty()) {
			...
			ctx.drawText(tr, Text.literal(msg),
		...
		for (int i = 0; i < cats.size(); i++) {
			...
			net.minecraft.item.ItemStack iconStack = new net.minecraft.item.ItemStack(cat.items().get(iconIdx));
			ctx.drawItem(iconStack, bx + 1, by + 1);
			if (hov) {
				// Tooltip shows only the name of the currently-drawn item (animated like the icon)
				int tipIdx = iconSlot % cat.items().size();
				net.minecraft.util.Identifier tipId = net.minecraft.registry.Registries.ITEM.getId(cat.items().get(tipIdx));
				String tipName = tipId != null ? formatName("modded:" + tipId) : cat.items().get(tipIdx).toString();
				ctx.drawTooltip(tr, Text.literal(tipName), mx, my);
			}
		}
	}
	...
	private void renderPhaseTwo(DrawContext ctx, int px, int py,
	                             int mx, int my, TextRenderer tr) {
		ctx.drawTexture(RenderLayer::getGuiTextured, P2_CONTAINER, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		...
		if (com.armaninyow.dibs.config.AeogConfig.showModButtonPhase2) {
			int modBtnAbsX = px + MOD_BTN_X, modBtnAbsY = py + MOD_BTN_Y;
			ctx.drawTexture(RenderLayer::getGuiTextured, isShowingModdedEnchants() ? BTN_MOD_H : BTN_MOD,
				modBtnAbsX, modBtnAbsY, 0f, 0f, MOD_BTN_SIZE, MOD_BTN_SIZE, MOD_BTN_SIZE, MOD_BTN_SIZE);
			if (inBounds(mx, my, modBtnAbsX, modBtnAbsY, MOD_BTN_SIZE, MOD_BTN_SIZE)) {
				ctx.drawTooltip(tr, Text.literal(isShowingModdedEnchants() ? "Hide Modded Enchants" : "Show Modded Enchants"), mx, my);
			}
		}
		...
		ctx.drawTexture(RenderLayer::getGuiTextured, calcTex, calcX, calcY, 0f, 0f, CALC_W, CALC_H, CALC_W, CALC_H);
		...
		for (int gi = 0; gi < incompatGroups.size(); gi++) {
			...
			if (expanded) {
				...
				for (int lv = 0; lv <= enchantMaxLevel(enchant); lv++) {
					...
					ctx.drawTexture(RenderLayer::getGuiTextured, lvTex, lvX, yTex, 0f, 0f, ROW_H, ROW_H, ROW_H, ROW_H);
				}
			} else {
				...
				if (group.size() > 1) {
					...
					ctx.drawTexture(RenderLayer::getGuiTextured, ph ? ENCH_PREV_H : ENCH_PREV,
				...
				ctx.drawTexture(RenderLayer::getGuiTextured, slotTex, slotX, yTex, 0f, 0f, SLOT_W, ROW_H, SLOT_W, ROW_H);
				if (group.size() > 1) {
					int nx = absX + PREV_W + SLOT_W;
					boolean nh = inBounds(mx, my, nx, yTex, NEXT_W, ROW_H);
					ctx.drawTexture(RenderLayer::getGuiTextured, nh ? ENCH_NEXT_H : ENCH_NEXT,
	...
	private void renderScrollBar(DrawContext ctx, int px, int py) {
		...
		ctx.drawTexture(RenderLayer::getGuiTextured, SCROLLER,
	...
	private void renderPhaseThreeTree(DrawContext ctx, int px, int py,
		...
		for (TreeNode node : treeNodes) {
			...
			ctx.drawTexture(RenderLayer::getGuiTextured, frame, nx, ny, 0f, 0f, NODE_SIZE, NODE_SIZE, NODE_SIZE, NODE_SIZE);
		...
		ctx.drawTexture(RenderLayer::getGuiTextured, P3_CONTAINER, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
	...
	private void renderPhaseThreeList(DrawContext ctx, int px, int py,
	                                   int mx, int my, TextRenderer tr) {
		// Container
		ctx.drawTexture(RenderLayer::getGuiTextured, P3_CONTAINER_2, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		renderBack(ctx, px, py, mx, my);
	...
	private void renderListRows(DrawContext ctx, TextRenderer tr,
		...
		for (ListRow row : rows) {
			...
			if (iconRowVisible) {
				...
				ctx.drawTexture(RenderLayer::getGuiTextured, obtained ? LIST_FRAME_O : LIST_FRAME_U,
	...
	private void renderTiledBg(DrawContext ctx, int x, int y) {
		...
		for (int ty = -bgH + offY; ty < P3_TREE_H; ty += bgH)
			for (int tx = -bgW + offX; tx < P3_TREE_W; tx += bgW)
				ctx.drawTexture(RenderLayer::getGuiTextured, BACKGROUND, x + tx, y + ty, 0f, 0f, bgW, bgH, bgW, bgH);
	}
			...
			for (ModdedCategory cat : s_moddedCategories) {
				if (cat.id().equals(resolvedId) && !cat.items().isEmpty()) {
					...
					net.minecraft.util.Identifier itemId =
						net.minecraft.registry.Registries.ITEM.getId(cat.items().get(idx));
	...
	private void renderNodeTooltip(DrawContext ctx, TreeNode node, boolean obtained, net.minecraft.item.ItemStack matchedStack, int mx, int my, TextRenderer tr) {
		...
		if (obtained && matchedStack != null && !matchedStack.isEmpty()) {
			nodeName = matchedStack.getName().getString();
		} else {
			nodeName = animatedNodeName(node.data().id());
		}
		if (!enchants.isEmpty()) {
			lines.add(Text.literal(nodeName).setStyle(nameStyle));
		} else {
			lines.add(Text.literal(nodeName));
		}
		...
		for (String[] e : enchants) {
			...
			lines.add(Text.literal(label).setStyle(style));
		}
	...
	private void renderBack(DrawContext ctx, int px, int py, int mx, int my) {
		...
		ctx.drawTexture(RenderLayer::getGuiTextured, hov ? BTN_BACK_H : BTN_BACK,
	...
	private void buildPhase2Data() {
		...
		if (mc2.world != null) {
			var enchantReg2 = mc2.world.getRegistryManager().getOrThrow(RegistryKeys.ENCHANTMENT);
			...
			if (vanillaStack != null && !vanillaStack.isEmpty()) {
				for (var enchEntry : enchantReg2.streamEntries().toList()) {
					net.minecraft.util.Identifier enchId = enchEntry.registryKey().getValue();
		...
		if (mode == com.armaninyow.dibs.config.AeogConfig.AutoFillMode.MAX_LEVELS) {
			...
			for (int i = 0; i < inv.size(); i++) {
				...
				for (String enchant : itemEnchants) {
					...
					net.minecraft.util.Identifier enchIdentifier = mcId.contains(":")
						? net.minecraft.util.Identifier.of(mcId)
						: Identifier.of("minecraft", mcId);
					var entry = reg.getEntry(enchIdentifier);
	...
	private String detectItemId(ItemStack s) {
		...
		if (s_moddedCategories != null) {
			for (ModdedCategory cat : s_moddedCategories) {
				for (net.minecraft.item.Item item : cat.items()) {
					if (s.isOf(item)) return cat.id();
				}
			}
		}
		return null;
	...
	private void sendCalculationRequest() {
		...
		for (String e : itemEnchants) {
			...
			if (EnchantData.ENCHANTS.containsKey(e)) {
				...
				int weight = 1;
				if (mcRef.world != null) {
					var reg = mcRef.world.getRegistryManager().getOrThrow(RegistryKeys.ENCHANTMENT);
			...
			if (enchComp != null && !enchants.isEmpty()) {
				...
				for (String[] e : enchants) {
					...
					net.minecraft.util.Identifier enchIdentifier = mcId.contains(":")
						? net.minecraft.util.Identifier.of(mcId)
						: Identifier.of("minecraft", mcId);
					var entryOpt = reg.getEntry(enchIdentifier);
	...
	private void drawIcon(DrawContext ctx, String itemId, int x, int y) {
			...
			if (s_moddedCategories != null) {
				for (ModdedCategory cat : s_moddedCategories) {
					if (cat.id().equals(itemId) && !cat.items().isEmpty()) {
						...
						ctx.drawItem(new net.minecraft.item.ItemStack(cat.items().get(iconIdx)), x, y);
						return;
					}
				}
			}
		...
		if (icon != null)
			ctx.drawTexture(RenderLayer::getGuiTextured, icon, x, y, 0f, 0f, ICON_SIZE, ICON_SIZE, ICON_SIZE, ICON_SIZE);
	}
