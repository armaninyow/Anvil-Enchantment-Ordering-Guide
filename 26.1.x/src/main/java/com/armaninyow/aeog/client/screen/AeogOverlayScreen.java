...
public class AeogOverlayScreen {
	...
	/**
	 * Maps treeNode index → true if this node is lIdx (left/target input) of its consumer step,
	 * false if it is rIdx (right/sacrifice input). Populated by buildTreeLayout().
	 * Key is absent for the final result node (no consumer).
	 */
	...
	private void renderListRows(GuiGraphicsExtractor ctx, Font tr,
		...
		for (ListRow row : rows) {
			...
			if (iconRowVisible) {
				...
				// Left icon — slot highlight back, then icon, then front
				...
				// Right icon — slot highlight back, then icon, then front
	...
	private void buildTreeLayout() {
		...
		for (int i = 0; i < steps; i++) {
			...
			// Record left/right role for each input node (used by click handler for slot routing)
	...
	private void clickPhase3(int px, int py, int mx, int my) {
		...
		if (com.armaninyow.dibs.config.AeogConfig.listViewPhase3) {
			...
			// Icon click: check each row's left and right icons
			Minecraft mc = Minecraft.getInstance();
			if (mc.player != null) {
				...
				for (ListRow row : rows) {
					...
					// Right icon — sacrifice slot (slot 1)
		...
		// Check if an obtained, non-final node was clicked
		for (int i = 0; i < treeNodes.size(); i++) {
			...
			ItemStack match = findMatchingStack(node.data(), node);
			if (match == null) return; // node hit but not obtained — consume click, no drag

			// Find the result node this node feeds into (via connectors),
			// then find its sibling (the other input to that result).
			// Visual left (smaller X) → target slot 0, visual right → sacrifice slot 1.
			int slot = 0; // default to target
			outer:
			for (Connector con : connectors) {
				if (con.fromIdx() != i) continue;
				int parentIdx = con.toIdx();
				// Find sibling: another connector pointing to the same parent
		...
		// No node clicked — start tree drag
		treeDragging = true;
	...
	/**
	 * Moves the given ItemStack from the player's inventory into the specified anvil slot.
	 * anvilSlot 0 = target (left), anvilSlot 1 = sacrifice (right).
	 *
	 * Uses handleInventoryMouseClick with QUICK_MOVE (shift-click) on the inventory slot
	 * while the anvil screen is open — the vanilla shift-click logic for ItemCombinerMenu
	 * always places items into slot 0 (target) first, then slot 1 (sacrifice).
	 * For precise slot control we instead do a two-step pickup+place sequence:
	 *   1. Left-click the inventory slot to pick the stack up onto the cursor.
	 *   2. Left-click the target anvil slot to place it there.
	 */
	private void sendToAnvilSlot(ItemStack stack, int anvilSlot) {
		...
		// If the anvil slot already has an item, shift-click it back to inventory first
		...
		// Find the inventory slot index that holds this exact stack reference
		...
		// Map inventory slot to container slot index.
		// ItemCombinerMenu layout: 0=target, 1=sacrifice, 2=result,
		// 3..29 = main inventory (inv slots 9-35), 30..38 = hotbar (inv slots 0-8)
		int containerSlot;
		if (invSlot < 9) {
			containerSlot = 30 + invSlot; // hotbar
		} else {
			containerSlot = invSlot - 6;  // main inventory (inv 9-35 → container 3-29)
		}

		// Step 1: left-click the inventory slot to pick up the stack
		mc.gameMode.handleContainerInput(
			containerId, containerSlot,
			0, net.minecraft.world.inventory.ContainerInput.PICKUP, mc.player);

		// Step 2: left-click the anvil slot to place it
		mc.gameMode.handleContainerInput(
			containerId, anvilSlot,
			0, net.minecraft.world.inventory.ContainerInput.PICKUP, mc.player);
	}
