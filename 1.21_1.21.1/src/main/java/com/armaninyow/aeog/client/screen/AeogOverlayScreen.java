...
public class AeogOverlayScreen {
	...
	private static List<ModdedCategory> buildModdedCategories() {
		...
		var enchantReg = mc.world.getRegistryManager().get(RegistryKeys.ENCHANTMENT);
	...
	private void playClick() {
		...
		mc.getSoundManager().play(new PositionedSoundInstance(
			net.minecraft.sound.SoundEvents.UI_BUTTON_CLICK.value().getId(),
	...
	private void renderPhaseOne(DrawContext ctx, int px, int py,
		...
		ctx.drawTexture(P1_CONTAINER, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		...
		if (com.armaninyow.dibs.config.AeogConfig.showModButtonPhase1) {
			...
			ctx.drawTexture(modMode ? BTN_MOD_H : BTN_MOD,
		...
		ctx.drawTexture(optHov ? BTN_OPT_H : BTN_OPT,
	...
	private void renderPhaseOneVanilla(DrawContext ctx, int px, int py,
		...
		for (int i = 0; i < items.size(); i++) {
			...
			ctx.drawTexture((hov || sel) ? BTN_ITEM_H : BTN_ITEM,
	...
	private void renderPhaseTwo(DrawContext ctx, int px, int py,
	                             int mx, int my, TextRenderer tr) {
		ctx.drawTexture(P2_CONTAINER, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		...
		if (com.armaninyow.dibs.config.AeogConfig.showModButtonPhase2) {
			int modBtnAbsX = px + MOD_BTN_X, modBtnAbsY = py + MOD_BTN_Y;
			ctx.drawTexture(isShowingModdedEnchants() ? BTN_MOD_H : BTN_MOD,
				modBtnAbsX, modBtnAbsY, 0f, 0f, MOD_BTN_SIZE, MOD_BTN_SIZE, MOD_BTN_SIZE, MOD_BTN_SIZE);
			if (inBounds(mx, my, modBtnAbsX, modBtnAbsY, MOD_BTN_SIZE, MOD_BTN_SIZE)) {
		...
		ctx.drawTexture(calcTex, calcX, calcY, 0f, 0f, CALC_W, CALC_H, CALC_W, CALC_H);
		...
		for (int gi = 0; gi < incompatGroups.size(); gi++) {
			...
			if (expanded) {
				...
				for (int lv = 0; lv <= enchantMaxLevel(enchant); lv++) {
					...
					ctx.drawTexture(lvTex, lvX, yTex, 0f, 0f, ROW_H, ROW_H, ROW_H, ROW_H);
				}
			} else {
				...
				if (group.size() > 1) {
					...
					ctx.drawTexture(ph ? ENCH_PREV_H : ENCH_PREV,
				...
				ctx.drawTexture(slotTex, slotX, yTex, 0f, 0f, SLOT_W, ROW_H, SLOT_W, ROW_H);
				if (group.size() > 1) {
					int nx = absX + PREV_W + SLOT_W;
					boolean nh = inBounds(mx, my, nx, yTex, NEXT_W, ROW_H);
					ctx.drawTexture(nh ? ENCH_NEXT_H : ENCH_NEXT,
	...
	private void renderPhaseThreeTree(DrawContext ctx, int px, int py,
		...
		for (TreeNode node : treeNodes) {
			...
			ctx.drawTexture(frame, nx, ny, 0f, 0f, NODE_SIZE, NODE_SIZE, NODE_SIZE, NODE_SIZE);
		...
		RenderSystem.enableBlend();
		RenderSystem.defaultBlendFunc();
		ctx.drawTexture(P3_CONTAINER, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		RenderSystem.disableBlend();
	...
	private void renderPhaseThreeList(DrawContext ctx, int px, int py,
	                                   int mx, int my, TextRenderer tr) {
		// Container
		ctx.drawTexture(P3_CONTAINER_2, px, py, 0f, 0f, P_W, P_H, P_W, P_H);
		renderBack(ctx, px, py, mx, my);
	...
	private void renderListRows(DrawContext ctx, TextRenderer tr,
		...
		for (ListRow row : rows) {
			...
			if (iconRowVisible) {
				...
				ctx.drawTexture(obtained ? LIST_FRAME_O : LIST_FRAME_U,
	...
	private void renderTiledBg(DrawContext ctx, int x, int y) {
		...
		for (int ty = -bgH + offY; ty < P3_TREE_H; ty += bgH)
			for (int tx = -bgW + offX; tx < P3_TREE_W; tx += bgW)
				ctx.drawTexture(BACKGROUND, x + tx, y + ty, 0f, 0f, bgW, bgH, bgW, bgH);
	}
	...
	private void renderBack(DrawContext ctx, int px, int py, int mx, int my) {
		...
		ctx.drawTexture(hov ? BTN_BACK_H : BTN_BACK,
	...
	private void buildPhase2Data() {
		...
		if (mc2.world != null) {
			var enchantReg2 = mc2.world.getRegistryManager().get(RegistryKeys.ENCHANTMENT);
	...
	private void sendCalculationRequest() {
		...
		for (String e : itemEnchants) {
			...
			if (EnchantData.ENCHANTS.containsKey(e)) {
				...
				int weight = 1;
				if (mcRef.world != null) {
					var reg = mcRef.world.getRegistryManager().get(RegistryKeys.ENCHANTMENT);
	...
	private void drawIcon(DrawContext ctx, String itemId, int x, int y) {
		...
		if (icon != null)
			ctx.drawTexture(icon, x, y, 0f, 0f, ICON_SIZE, ICON_SIZE, ICON_SIZE, ICON_SIZE);
	}
